...
import {addCreditsByKeyHash,markStripeEvent} from "@/lib/store"
export async function POST(req:Request){
 ...
 try{
  ...
  if(event.type==="checkout.session.completed"){
   ...
   if(credits>0&&keyHash){const fresh=await markStripeEvent(event.id); if(fresh)await addCreditsByKeyHash(keyHash,credits)}
  }
